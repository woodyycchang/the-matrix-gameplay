...
(function (G) {
  ...
  function sceneHallway() {
    ...
    var farCol = box2([-HL - 0.6, 0, -HW], [-HL + 0.12, H, HW]);                    // far end (lifted at the seal)
    s.colliders.push(farCol);
      ...
      if (dv.phase === 'settle') {
        ...
        if (dv.g >= HALL.SEAL_AT) {
          ...
          for (var sk = 0; sk < 3; sk++) {
            ...
            seg.loadT = 0; seg.loadDir = 1; seg._delay = 0.3 + sk * 0.12;
            s.insts.push(seg);
            var sl = inst(C.props.ceilLamp(false), [scx, H, 0], 0, { label: 'lamp', kind: 'segment', shadow: false });
            sl.state = 'lit'; sl.loadT = 0; sl.loadDir = 1; sl._delay = 0.35 + sk * 0.12;
            s.insts.push(sl);
