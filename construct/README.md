...
- **"a hallway"** (or **"deja vu"**) — a narrow apartment corridor. A black cat crosses a lit doorway; seconds later the same walk plays again, frame for frame, off a recording. Between the passes exactly one fixture — picked from the scene graph at that moment — genuinely changes state, and in code vision its glyphs re-resolve so you can verify. When the replay ends, the way you came in is bricked up — and the far door, the one that never opens, opens onto a corridor that repeats forever — every period hangs the same door, and each one swings open as you come near, then eases shut behind you. Run as far as you like; the phone stays a finite walk behind you.
