...
- Tests: `node node/tests.js` — **1040 PASS / 0 FAIL** at handoff. DE-NAG LAW: Game.say drops identical lines within 30 s; mobil exit flavor once per SESSION; empire plank announces removed (letters carry it); peach cut-lines only at 1 and 6. C.VER on the HUD (bump every push). AMENDED COVENANT: scene/clear requests bypass scene onWord (mobil can never lock the operator; props still unravel there; exit flavor once per visit). body.dark is DECLARED via scene.uiDark (census: only neon mile) - the /NEON/ name-regex is retired. Theme #15: PEACH BLOSSOM SPRING (word chain peach blossom|blossom|peach|wuling) - English-only by decree, the whole fable is one E2E court (board/row/squeeze/five talks/warned once/6 cuts/report/marks vanish/cliff seals/epilogue). Skiff via auto-ashore (bian she chuan). No melody ported (music law). The ORIGINAL epang (Three.js) lives at /epang-palace/ (subtree; root untouched). EPANG carries all twelve stagings + stateless-particle motion (report 4.5) + fog-sky one-scalar law; E6.1-E6.6 probes are the acceptance courts. Deferred: F7-full (whole skyway deck above the hall ridge; needs a switchback stair - the arch + raised roof carry the line meanwhile). Feature tree: `eval/tree.json` enforced by `node/tree_guard.js` (rules R1–R5: every node needs evidence; branches close only when children close; etc.). Tree state at handoff: **ROOT = PASS, zero open nodes**.
...
**14 keyword-triggered themes** (lexical rows in 06 + chips in 08 + INTENTS in 09 + L lines in 06): weapons, dojo, rooftop, motorcycle, katana, city street, **neon** (mile), **hallway**, **erebus** (station), **epang** (palace), **empire** (Angel City 1937), **mobil** (Mobil Ave / limbo), a chair, clear.
...
- **ORANGE EMPIRE 1937 (Angel City)** — cross-engine port of orange-empire-1937, distances 1:1 (BOUNDARY 965, WALL 1000, barricades 300/520/700, shack 335, truck 760). Main St (14 measured-color facades, lit windows, RIALTO blade, BLUE PACIFIC cyan + martini strokes), walk-in bar (Gus, phonograph, radio), EAST GROVE ROAD as finite streamed chunks (per-chunk deterministic orchard, decay baked per band, solid-tree colliders travel with the chunk; wire trees past 665 are intangible green), 3 breakable barricades (>5.5 m/s, their law), Whitmore shack + the J.W. note verbatim, wire-edged dead truck, green-grid wall arc. **C.CAR** = branch CFG verbatim (VMAX 26/ACC 9.5/BRAKE 16/STEER 2.0 speed-scaled), mount/dismount/moveCar parallel to the bike; generic **scene.onWord** hook in Game.request lets an armed scene claim the console. Six seams (USUAL / SKIP 64 s needle-crackle / EVERGREEN / CLOCKWORK quarter-past / MIRROR twins / NINE-SIX-FIVE beeps). ENDING at 965: engine dismissed, 7 lines + MERIDIAN terminal verbatim, console = terminal: RETURN loops to 7:24 (memory retained, Gus asks how green), END wakes in the void. Sky 'empire' = their 3-palette day on a loop-resettable **scene.skyT** minute clock (engine sky op learns scene.skyT); the town's evening is BAKED (no lights in this renderer) and night is a SECOND MESH swapped by the mutation trick (bar interior + vivid faces exempt). Audio: ambience 'empire', thud/sting/crackle (no scores, no voices). Five visual-parity passes + one shutter-audit correction logged in eval/orange-port.md.
- **MOBIL AVE** (claude/mobil-ave-3d port) - the between-place as a literal 288 m ring (R=45.84, their spec 1:1): platform tiles [6,42), dot-matrix MOBIL, benches/pillars/rails, their light rig baked (9 sodium + sign wash + whites, the fixture over the doors is the dark one). THE RULE: scene.onWord intercepts everything - requests sketch a pedestal that unravels, exits get static, clear does not compile. The train (40 s: 24 arrive/27 open/33 seal/35 depart) is one curved mesh ROTATED about the ring center (it.yaw IS motion); per-tile arc colliders splice on arrive/depart; body leaves the doorway to the door panels; walking through open doors -> void. Scramble-by-containment if caught on the rails. All lines verbatim from construct/mobilAve.js.
- **tools/selfshot.js** — the headless camera: renders any scene to PNG via the game's own `C.render` ops + a scanline painter. Supports **per-pose scene word, TIME override, 90-step materialize settle**, and REAL sky painters (epang day-cycle + Shu ridge; erebus starfield). Twelve poses: epang(dawn t=12)/dock/rotunda/reactor/bridge/dome/empire_dusk/empire_night/empire_wire (mobil poses take pose[6]=cycle-seconds via s._t0; empire poses take pose[6]=dayMin and settle 100 steps after turning the day knob — the night mesh-swap re-materializes the town). Companion judges: histogram stats (lum / distinct colors / warm% threshold **28** / cyan-glow) and 44×13 ASCII color maps. Rig B′ (`/home/claude/rigb/shoot.js` pattern, not in repo) can software-render THREE.js branch games via Proxy-stub + THREE-as-math.
