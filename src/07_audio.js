...
(function (G) {
  ...
  A.setAmbience = function (name) {
    ...
    setTimeout(function () {
      ...
      ambGain.gain.linearRampToValueAtTime(name === 'void' ? 0.1 : 0.22, t1 + 1.4);
    }, 220);
  };
  ...
  A.handle = function (name) {
    ...
    switch (name) {
      ...
      case 'tick': blip(1900 + Math.random() * 400, 0.018, 0.05, 'square'); break;
  ...
  A.speak = function (text) {
    ...
    try {
      ...
      u.rate = 0.85; u.pitch = 0.5; u.volume = 1.0;   // "voice-of-god" recipe: deep pitch + slowed 0.85 (pro trailer-narrator config, mapped to the 0-2 browser scale)
