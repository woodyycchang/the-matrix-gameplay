...
(function (G) {
  ...
  var VOICE_WORKER_SRC = [
    ...
    "async function load() {",
    "  try { tts = await KokoroTTS.from_pretrained(MODEL, { dtype: 'q8', device: 'webgpu' }); postMessage({ type: 'ready', device: 'webgpu' }); return; } catch (e) {}",
    "  try { tts = await KokoroTTS.from_pretrained(MODEL, { dtype: 'q8', device: 'wasm' }); postMessage({ type: 'ready', device: 'wasm' }); }",
    "  catch (e2) { postMessage({ type: 'fail', error: String((e2 && e2.message) || e2) }); }",
    "}",
    "onmessage = async (ev) => {",
    "  const m = ev.data || {};",
  ...
  function loadVoice() {
    ...
    vox.worker.onmessage = function (ev) {
      ...
      if (m.type === 'ready') {
        vox.state = 'on';
        A.ttsReady = true;
        A.voiceName = 'Kokoro am_adam (AI \u00b7 ' + (m.device || '') + ')';
